				...
				<div class="feature grid-item">
					<div class="feature__image">
						<?php if ( 'icon_font' == $item->item_type ) : ?>
							<?php $icon_color = ! empty( $item->icon_color ) ? 'style="color:'.$item->icon_color.'"' : ''; ?>
							<?php if ( ! empty( $item->icon ) ) : ?>
								<i class="<?php echo ''.$item->icon; ?>" <?php echo ''.$icon_color; ?>></i>
							<?php else :  ?>
								<i class="ti-truck" <?php echo ''.$icon_color; ?>></i>
							<?php endif; ?>
						<?php else : ?>
							<?php $get_img_src = wp_get_attachment_image_src( $item->image, 'thumbnail' ); ?>
							<?php if ( ! empty( $get_image_src ) ) : ?>
								<img src="<?php echo esc_url( $get_image_src[0] ); ?>" alt="<?php esc_html_e( 'Features Image', 'multimarket' ); ?>">
							<?php endif; ?>
						<?php endif; ?>
					</div>
