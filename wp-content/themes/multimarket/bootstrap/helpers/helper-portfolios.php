 ...
 * @author tokoo
 **/
function multimarket_portfolio_get_client_name() {
	$meta 			= get_post_meta( get_the_ID(), 'multimarket_project_details', true );
 ...
 * @author tokoo
 **/
function multimarket_portfolio_get_url() {
	$meta 	= get_post_meta( get_the_ID(), 'multimarket_project_details', true );
 ...
 * @author tokoo
 **/
function multimarket_portfolio_get_gallery_ids() {
	$meta 		= get_post_meta( get_the_ID(), 'multimarket_project_details', true );
 ...
 * @author tokoo
 **/
function multimarket_portfolio_gallery_images( $before_image = '', $after_image = '', $image_size = 'medium' ) {
	...
	if ( ! empty( $galleries ) ) {
		foreach ( $galleries as $gallery ) {
			...
			printf( $before_image );
			echo '<img src="'.esc_url( $image_url[0] ).'" alt="'.esc_html__( 'Project Gallery', 'multimarket' ).'">';
			printf( $after_image );
 ...
 * @author tokoo
 **/
function multimarket_portfolio_get_categories() {
 ...
 * @author tokoo
 **/
function multimarket_portfolio_get_tags() {
