...
class Multimarket_ThemeForest_Item extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname' 	=> 'paradize-themeforest-item',
			'description' 	=> esc_html__( 'Display ThemeForest items with this widget', 'multimarket' )
		);
		...
		parent::__construct( 'multimarket_ThemeForest_Item', esc_html__( 'Tokoo - Themeforest Item', 'multimarket' ), $widget_ops, $control_ops );
		...
		$this->defaults = array(
			...
			'more_link_txt' => esc_html__( 'View more', 'multimarket' ),
			'order' 		=> 'desc',
			'target' 		=> '_blank',
			'exclude' 		=> ''
		);
		...
		$this->defaults = apply_filters( 'multimarket_tf_widget_modify_defaults', $this->defaults );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php
			if ( ! empty( $instance['more_link_url'] ) ) :
				$more_text = isset( $instance['more_link_txt'] ) && ! empty( $instance['more_link_txt'] ) ? $instance['more_link_txt'] : esc_html__( 'View more', 'multimarket' ); ?>
	...
	function form($instance) {
		...
		esc_html_e( 'Title', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		esc_html_e( 'Description', 'multimarket' ); ?>:</label>
			<textarea id="<?php
		...
		<p>
			<label"><?php
		esc_html_e( 'Item categories to show', 'multimarket' ); ?>:</label><br/>
			<?php
	  ...
	  <p>
			<label"><?php
		esc_html_e( 'Select items from', 'multimarket' ); ?>:</label><br/>
			<input id="<?php
		...
		esc_html_e( 'Popular Items (WordPress Only)', 'multimarket' ); ?></label><br/>
			<input id="<?php
		...
		esc_html_e( 'Latest Items', 'multimarket' ); ?></label><br/>
			<input id="<?php
		...
		esc_html_e( 'Specific User(s)', 'multimarket' ); ?></label>
	  </p>

		<p>
			<label for="<?php
		echo esc_attr( $this->get_field_id( 'user' ) ); ?>"><?php
		esc_html_e( 'ThemeForest username(s)', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		esc_html_e( 'For multiple users, separate by comma: i.e. user1,user2,user3', 'multimarket' ); ?></small>
		</p>
		...
		esc_html_e( 'Number of items to show', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		<p>
			<label><?php
		esc_html_e( 'Order by', 'multimarket' ); ?>:</label>
				<select id="<?php
		...
		esc_html_e( 'Ascending', 'multimarket' ); ?></label>
			<input id="<?php
		...
		esc_html_e( 'Descending', 'multimarket' ); ?></label>
		</p>

		<p>
			<label for="<?php
		echo esc_attr( $this->get_field_id( 'exclude' ) ); ?>"><?php
		esc_html_e( 'Exclude item(s)', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		echo strip_tags($instance['exclude']); ?>" class="widefat" />
		  <small class="howto"><?php
		esc_html_e( 'Specify item ID to exclude specific item (separate by comma for multiple items): i.e. 8134834,7184572', 'multimarket' ); ?></small>
		</p>

		<p>
			<label for="<?php
		echo esc_attr( $this->get_field_id( 'ref' ) ); ?>"><?php
		esc_html_e( 'Referral user', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		echo strip_tags( $instance['ref'] ); ?>" class="widefat" />
			<small class="howto"><?php
		esc_html_e( 'Specify username if you want to use items as ThemeForest affiliate links', 'multimarket' ); ?></small>
		</p>

		<p>
			<label for="<?php
		echo esc_attr( $this->get_field_id( 'more_link_url' ) ); ?>"><?php
		esc_html_e( 'More link URL', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		echo esc_attr($instance['more_link_url']); ?>" class="widefat" />
			<small class="howto"><?php
		esc_html_e( 'Specify URL if you want to show "more" link under the items list', 'multimarket' ); ?></small>
		</p>

		<p>
			<label for="<?php
		echo esc_attr( $this->get_field_id( 'more_link_txt' ) ); ?>"><?php
		esc_html_e( 'More link text', 'multimarket' ); ?>:</label>
			<input id="<?php
		...
		esc_html_e( 'Open items in', 'multimarket' ); ?>: </label>
			<select id="<?php
		...
		esc_html_e( 'New Window', 'multimarket' ); ?></option>
				<option value="_self" <?php selected( '_self', $instance['target'] ); ?>><?php
		esc_html_e( 'Same Window', 'multimarket' ); ?></option>
			</select>
		</p>
