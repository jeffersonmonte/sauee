...
class Multimarket_Flickr_Photo_Slides extends Tokoo_Widget {
	...
	function __construct() {
		...
		$args = array(
			'label' 		=> esc_html__( 'Tokoo - Flickr Photo Slides', 'multimarket' ),
			'description' 	=> esc_html__( 'A custom widget to display the flickr photo in slide display.', 'multimarket' ),
		 );
		...
		$args['fields'] = array(
			...
			array(
				...
				'type' 		=> 'text',
				'class' 	=> 'widefat',
				'std' 		=> esc_html__( 'Flickr Photo Slides', 'multimarket' ),
				'validate' 	=> 'alpha_dash',
	...
	function get_photos( $id, $count = 8 ) {
		...
		$transient_key 	= md5( 'multimarket_flickr_slides_cache_' . $id . $count );
