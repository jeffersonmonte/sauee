 ...
 * @author tokoo
 **/
function multimarket_page_section_title() {
	$page_meta 					= multimarket_get_meta( '_page_details' );
	$section_title 				= ( isset( $page_meta['perpage_page_title'] ) && ! empty( $page_meta['perpage_page_title'] ) ) ? $page_meta['perpage_page_title'] : '';
	$section_subtitle 			= ( isset( $page_meta['perpage_page_subtitle'] ) && ! empty( $page_meta['perpage_page_subtitle'] ) ) ? $page_meta['perpage_page_subtitle'] : '';
	
	if ( ! empty( $page_meta['perpage_page_title'] ) ) : ?>
		
		<div class="section-header">
			<h2 class="section-header__title"><?php echo esc_attr( $section_title ); ?></h2>
			<small class="section-header__subtitle"><?php echo esc_attr( $section_subtitle ); ?></small>
		</div>
 ...
 * @author tokoo
 **/
function multimarket_data_pagination_loop() {
	...
	<?php if ( get_next_posts_link() ) : ?>
		<?php next_posts_link( wp_kses( __( 'Next <i class="fa fa-angle-right"></i>', 'multimarket' ), array( 'i' => array( 'class' => array() ) ) ) ); ?>
	<?php endif; ?>

	<?php if ( get_previous_posts_link() ) : ?>
		<?php previous_posts_link( wp_kses( __( '<i class="fa fa-angle-left"></i> Previous', 'multimarket' ), array( 'i' => array( 'class' => array() ) ) ) ); ?>
	<?php endif; 
}
 ...
 * @author tokoo
 **/
function multimarket_data_breadcrumbs() {
	...
	$curr_after		= '</span>';
	$home 			= _x( 'Home', 'breadcrumb', 'multimarket' );
	...
	if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && ( function_exists( 'wc_get_page_id' ) && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) ) || is_paged() ) {
		...
		if ( is_category() ) {
			...
			echo ''.$prepend . $before . $curr_before . esc_html__( 'Products tagged &ldquo;', 'multimarket' ) . $queried_object->name . '&rdquo;' . $curr_after . $after;
			...
			if ( is_search() ) {
				...
				echo ''.$before . '<a href="' . esc_url( get_post_type_archive_link( 'product' ) ) . '">' . $_name . '</a>' . $delimiter . $curr_before . esc_html__( 'Search results for &ldquo;', 'multimarket' ) . get_search_query() . '&rdquo;' . $curr_after . $after;
			...
			echo ''.$before . $curr_before . esc_html__( 'Error 404', 'multimarket' ) . $curr_after . $after;
			...
			echo ''.$before . $curr_before . esc_html__( 'Search results for &ldquo;', 'multimarket' ) . get_search_query() . '&rdquo;' . $curr_after . $after;

		} elseif ( is_tag() ) {

				echo ''.$before . $curr_before . esc_html__( 'Posts tagged &ldquo;', 'multimarket' ) . single_tag_title('', false) . '&rdquo;' . $curr_after . $after;

		} elseif ( is_author() ) {

			$userdata = get_userdata( get_queried_object_id () );
			echo ''.$before . $curr_before . esc_html__( 'Author:', 'multimarket' ) . ' ' . $userdata->display_name . $curr_after . $after;

		}

		if ( get_query_var( 'paged' ) )
			echo ''. $curr_before . ' (' . esc_html__( 'Page', 'multimarket' ) . ' ' . get_query_var( 'paged' ) . ')' . $curr_after;
