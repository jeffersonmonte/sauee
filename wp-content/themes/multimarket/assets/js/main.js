...
//- multimarket settings variable from wp localized
var multimarket = multimarket_js_var;
...
function initProductCarousel() {
	var el = $('.mm-products-carousel');
	var carousel = el.find('.grid-layout');
	var visible = el.data('visible');
	var isAuto = el.data('auto');
	var delay = el.data('autodelay');
	var isOffset = el.data('isoffset');
	var offset = el.data('offset');

	carousel.slick({
		slidesToShow: parseInt(visible),
		centerMode: isOffset,
		centerPadding: parseInt(offset) + 'px',
		dots: true,
		prevArrow: '<button type="button" class="slick-prev">Previous</button>',
		nextArrow: '<button type="button" class="slick-next">Next</button>',
		autoplay: isAuto,
		autoplaySpeed: parseInt(delay),
		responsive: [{
			breakpoint: 990,
			settings: {
				slidesToShow: 3,
...
function testimonialSlider() {
	$('.testimonial-slider').slick({
		slidesToShow: 1,
		prevArrow: '<button class="no-ui slick-prev"><i class="fa fa-angle-left"></i></button>',
		nextArrow: '<button class="no-ui slick-next"><i class="fa fa-angle-right"></i></button>',
		responsive: [{
			breakpoint: 480,
			settings: {
				slidesToShow: 1,
...
function initGoodies() {
	...
	//- Page Header
	// let pageHeader = new ScrollMagic.Scene({
	// 	triggerHook    : 0,
...
function initMap() {
	$(".tokoo-map").each(function () {
		...
		mapEl.gmap3({
			...
			styles: style
		}).marker({ position: [lat, lon] }).infowindow({
			content: "<div class='tokoo-info-window'><h2>" + title + "</h2>" + content + "</div>"
...
function initDecoration() {
	if ($('.mm-product-action').length) {
		var headerHeight = $(".site-header").innerHeight();
		var actionOffset = $(".mm-product-action").offset().top;
		var actionHeight = $(".mm-product-action").innerHeight();
		var isAdminBar = $(".admin-bar").length ? 30 : 0;

		$(".mm-top-bg").height(actionOffset + actionHeight - headerHeight - isAdminBar).addClass("loaded");
	}
}
...
$(document).ready(function () {
	...
	if ($('#kc-element-placeholder').length) {
		$(".body").addClass('liveeditor-enabled');
	}
